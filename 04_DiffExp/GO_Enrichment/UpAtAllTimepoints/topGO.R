...
for (GOgroup in c("BP", "MF", "CC")) {
	...
	write.table(allRes, paste0("UpInAll.", GOgroup, ".txt"), quote = FALSE, sep = "\t", row.names = FALSE)
	#showSigOfNodes(GOdata, score(resultFis), firstSigNodes = 5, useInfo = 'all')
	printGraph(GOdata, resultFis, firstSigNodes = 5, fn.prefix = paste0("UpInAll.", GOgroup),

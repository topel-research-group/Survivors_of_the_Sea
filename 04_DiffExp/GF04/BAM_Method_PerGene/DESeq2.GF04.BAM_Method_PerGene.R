...
coldata <- read.delim("../GF04_samples.tsv", row.names=1, stringsAsFactors=TRUE)
...
tx2gene <- read.csv("../GF04_tx2gene.lst", sep="\t", header=FALSE)
...
print(paste0("Number of genes: ", nrow(dds)))		# 33,975
...
print(paste0("Number of genes with one count in at least three samples: ", nrow(dds)))	# 26,213
...
#png("GF04.BAM_Method_PerGene.Heatmap.png")
#pheatmap(distMatrix_rld, annotation_col = annotation_col)
#dev.off()

## PCA

#png("GF04.BAM_Method_PerGene.PCA.png")
...
print(paste0("0d vs 49d - genes with adjusted p-value: ", nrow(res49)))         # 21,131
res56 <- res56[ !is.na(res56$padj), ]
print(paste0("0d vs 56d - genes with adjusted p-value: ", nrow(res56)))         # 21,639
res72 <- res72[ !is.na(res72$padj), ]
print(paste0("0d vs 72d - genes with adjusted p-value: ", nrow(res72)))         # 22,655
res91 <- res91[ !is.na(res91$padj), ]
print(paste0("0d vs 91d - genes with adjusted p-value: ", nrow(res91)))         # 20,622
res126 <- res126[ !is.na(res126$padj), ]
print(paste0("0d vs 126d - genes with adjusted p-value: ", nrow(res126)))       # 21,131
res189 <- res189[ !is.na(res189$padj), ]
print(paste0("0d vs 189d - genes with adjusted p-value: ", nrow(res189)))       # 20,622

res49_56 <- res49_56[ !is.na(res49_56$padj), ]
print(paste0("49d vs 56d - genes with adjusted p-value: ", nrow(res49_56)))     ## 19,606
res49_72 <- res49_72[ !is.na(res49_72$padj), ]
print(paste0("49d vs 72d - genes with adjusted p-value: ", nrow(res49_72)))     ## 18,081
res49_91 <- res49_91[ !is.na(res49_91$padj), ]
print(paste0("49d vs 91d - genes with adjusted p-value: ", nrow(res49_91)))     ## 18,081
res49_126 <- res49_126[ !is.na(res49_126$padj), ]
print(paste0("49d vs 126d - genes with adjusted p-value: ", nrow(res49_126)))   ## 19,098
res49_189 <- res49_189[ !is.na(res49_189$padj), ]
print(paste0("49d vs 189d - genes with adjusted p-value: ", nrow(res49_189)))   ## 19,098

res56_72 <- res56_72[ !is.na(res56_72$padj), ]
print(paste0("56d vs 72d - genes with adjusted p-value: ", nrow(res56_72)))     ## 17,065
res56_91 <- res56_91[ !is.na(res56_91$padj), ]
print(paste0("56d vs 91d - genes with adjusted p-value: ", nrow(res56_91)))     ## 19,606
res56_126 <- res56_126[ !is.na(res56_126$padj), ]
print(paste0("56d vs 126d - genes with adjusted p-value: ", nrow(res56_126)))   ## 19,606
res56_189 <- res56_189[ !is.na(res56_189$padj), ]
print(paste0("56d vs 189d - genes with adjusted p-value: ", nrow(res56_189)))   ## 20,114

res72_91 <- res72_91[ !is.na(res72_91$padj), ]
print(paste0("72d vs 91d - genes with adjusted p-value: ", nrow(res72_91)))     ## 17,065
res72_126 <- res72_126[ !is.na(res72_126$padj), ]
print(paste0("72d vs 126d - genes with adjusted p-value: ", nrow(res72_126)))   ## 17,573
res72_189 <- res72_189[ !is.na(res72_189$padj), ]
print(paste0("72d vs 189d - genes with adjusted p-value: ", nrow(res72_189)))   ## 19,098

res91_126 <- res91_126[ !is.na(res91_126$padj), ]
print(paste0("91d vs 126d - genes with adjusted p-value: ", nrow(res91_126)))   ## 14,016
res91_189 <- res91_189[ !is.na(res91_189$padj), ]
print(paste0("91d vs 189d - genes with adjusted p-value: ", nrow(res91_189)))   ## 16,557

res126_189 <- res126_189[ !is.na(res126_189$padj), ]
print(paste0("126d vs 189d - genes with adjusted p-value: ", nrow(res126_189))) ## 15,540

###

sig49 <- res49[which(res49$padj < 0.05), ]
print(paste0("0d vs 49d - genes with adjusted p-value < 0.05: ", nrow(sig49)))          # 14.225
sig49_sorted <- sig49[order(sig49$log2FoldChange),]
sig56 <- res56[which(res56$padj < 0.05), ]
print(paste0("0d vs 56d - genes with adjusted p-value < 0.05: ", nrow(sig56)))          # 12,780
sig56_sorted <- sig56[order(sig56$log2FoldChange),]
sig72 <- res72[which(res72$padj < 0.05), ]
print(paste0("0d vs 72d - genes with adjusted p-value < 0.05: ", nrow(sig72)))          # 13,437
sig72_sorted <- sig72[order(sig72$log2FoldChange),]
sig91 <- res91[which(res91$padj < 0.05), ]
print(paste0("0d vs 91d - genes with adjusted p-value < 0.05: ", nrow(sig91)))          # 13,226
sig91_sorted <- sig91[order(sig91$log2FoldChange),]
sig126 <- res126[which(res126$padj < 0.05), ]
print(paste0("0d vs 126d - genes with adjusted p-value < 0.05: ", nrow(sig126)))        # 13,615
sig126_sorted <- sig126[order(sig126$log2FoldChange),]
sig189 <- res189[which(res189$padj < 0.05), ]
print(paste0("0d vs 189d - genes with adjusted p-value < 0.05: ", nrow(sig189)))        # 13,012
sig189_sorted <- sig189[order(sig189$log2FoldChange),]

sig49_56 <- res49_56[which(res49_56$padj < 0.05), ]
print(paste0("49d vs 56d - genes with adjusted p-value < 0.05: ", nrow(sig49_56)))      ## 9,270
sig49_56_sorted <- sig49_56[order(sig49_56$log2FoldChange),]
sig49_72 <- res49_72[which(res49_72$padj < 0.05), ]
print(paste0("49d vs 72d - genes with adjusted p-value < 0.05: ", nrow(sig49_72)))      ## 8,244
sig49_72_sorted <- sig49_72[order(sig49_72$log2FoldChange),]
sig49_91 <- res49_91[which(res49_91$padj < 0.05), ]
print(paste0("49d vs 91d - genes with adjusted p-value < 0.05: ", nrow(sig49_91)))      ## 7,748
sig49_91_sorted <- sig49_91[order(sig49_91$log2FoldChange),]
sig49_126 <- res49_126[which(res49_126$padj < 0.05), ]
print(paste0("49d vs 126d - genes with adjusted p-value < 0.05: ", nrow(sig49_126)))    ## 8,063
sig49_126_sorted <- sig49_126[order(sig49_126$log2FoldChange),]
sig49_189 <- res49_189[which(res49_189$padj < 0.05), ]
print(paste0("49d vs 189d - genes with adjusted p-value < 0.05: ", nrow(sig49_189)))    ## 7,552
sig49_189_sorted <- sig49_189[order(sig49_189$log2FoldChange),]

sig56_72 <- res56_72[which(res56_72$padj < 0.05), ]
print(paste0("56d vs 72d - genes with adjusted p-value < 0.05: ", nrow(sig56_72)))      ## 4,625
sig56_72_sorted <- sig56_72[order(sig56_72$log2FoldChange),]
sig56_91 <- res56_91[which(res56_91$padj < 0.05), ]
print(paste0("56d vs 91d - genes with adjusted p-value < 0.05: ", nrow(sig56_91)))      ## 6,486
sig56_91_sorted <- sig56_91[order(sig56_91$log2FoldChange),]
sig56_126 <- res56_126[which(res56_126$padj < 0.05), ]
print(paste0("56d vs 126d - genes with adjusted p-value < 0.05: ", nrow(sig56_126)))    ## 7,897
sig56_126_sorted <- sig56_126[order(sig56_126$log2FoldChange),]
sig56_189 <- res56_189[which(res56_189$padj < 0.05), ]
print(paste0("56d vs 189d - genes with adjusted p-value < 0.05: ", nrow(sig56_189)))    ## 8,385
sig56_189_sorted <- sig56_189[order(sig56_189$log2FoldChange),]

sig72_91 <- res72_91[which(res72_91$padj < 0.05), ]
print(paste0("72d vs 91d - genes with adjusted p-value < 0.05: ", nrow(sig72_91)))      ## 4,598
sig72_91_sorted <- sig72_91[order(sig72_91$log2FoldChange),]
sig72_126 <- res72_126[which(res72_126$padj < 0.05), ]
print(paste0("72d vs 126d - genes with adjusted p-value < 0.05: ", nrow(sig72_126)))    ## 4,678
sig72_126_sorted <- sig72_126[order(sig72_126$log2FoldChange),]
sig72_189 <- res72_189[which(res72_189$padj < 0.05), ]
print(paste0("72d vs 189d - genes with adjusted p-value < 0.05: ", nrow(sig72_189)))    ## 6,845
sig72_189_sorted <- sig72_189[order(sig72_189$log2FoldChange),]

sig91_126 <- res91_126[which(res91_126$padj < 0.05), ]
print(paste0("91d vs 126d - genes with adjusted p-value < 0.05: ", nrow(sig91_126)))    ## 999
sig91_126_sorted <- sig91_126[order(sig91_126$log2FoldChange),]
sig91_189 <- res91_189[which(res91_189$padj < 0.05), ]
print(paste0("91d vs 189d - genes with adjusted p-value < 0.05: ", nrow(sig91_189)))    ## 3,923
sig91_189_sorted <- sig91_189[order(sig91_189$log2FoldChange),]

sig126_189 <- res126_189[which(res126_189$padj < 0.05), ]
print(paste0("126d vs 189d - genes with adjusted p-value < 0.05: ", nrow(sig126_189)))  ## 2,579
...
# transcriptome <- read.table("../GF04.emapper.annotations", sep="\t", row.names = 1, quote="", header=TRUE)
...
print(paste0("0d vs 49d - genes upregulated in dormancy: ", nrow(sig49_DormantUp)))		# 6,875
...
print(paste0("0d vs 49d - genes downregulated in dormancy: ", nrow(sig49_DormantDown)))		# 7,350
...
print(paste0("0d vs 56d - genes upregulated in dormancy: ", nrow(sig56_DormantUp)))		# 5,967
...
print(paste0("0d vs 56d - genes downregulated in dormancy: ", nrow(sig56_DormantDown)))		# 6,813
...
print(paste0("0d vs 72d - genes upregulated in dormancy: ", nrow(sig72_DormantUp)))		# 6,318
...
print(paste0("0d vs 72d - genes downregulated in dormancy: ", nrow(sig72_DormantDown)))		# 7,119
...
print(paste0("0d vs 91d - genes upregulated in dormancy: ", nrow(sig91_DormantUp)))		# 6,328
...
print(paste0("0d vs 91d - genes downregulated in dormancy: ", nrow(sig91_DormantDown)))		# 6,898
...
print(paste0("0d vs 126d - genes upregulated in dormancy: ", nrow(sig126_DormantUp)))		# 6,571
...
print(paste0("0d vs 126d - genes downregulated in dormancy: ", nrow(sig126_DormantDown)))	# 7,044
...
print(paste0("0d vs 189d - genes upregulated in dormancy: ", nrow(sig189_DormantUp)))		# 6,386
...
print(paste0("0d vs 189d - genes downregulated in dormancy: ", nrow(sig189_DormantDown)))	# 6,626

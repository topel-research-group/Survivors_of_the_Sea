...
rld_vr <- rlog(dds_vr, blind = FALSE)
...
sum(is.na(res$padj))		# 377
sum(is.na(res_vr$padj))		# 0

res <- res[ !is.na(res$padj), ]			# 19,056
res_vr <- res_vr[ !is.na(res_vr$padj), ]	# 17,983

sig <- res[which(res$padj < 0.01), ]		# 11,387
sig_vr <- res_vr[which(res_vr$padj < 0.01), ]	# 11,732

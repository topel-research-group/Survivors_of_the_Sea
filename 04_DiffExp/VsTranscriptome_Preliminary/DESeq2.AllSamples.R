...
coldata <- read.delim("../../SampleInfo.tsv", row.names=1, stringsAsFactors=TRUE)
...
coldata$group <- paste(coldata$Strain,coldata$Dormancy, sep="_")
...
dds <- DESeqDataSetFromTximport(txi, coldata, ~Batch + group)
...
annotation_col <- coldata[,c(3,4,5)]
...
plotPCA(rld_blind, intgroup = c("group","Batch"))
plotPCA(vsd, intgroup = c("group","Batch"))
plotPCA(vsd_blind, intgroup = c("group","Batch"))
...
sum(is.na(res$padj))		# 167

res <- res[ !is.na(res$padj), ]	# 20,608

sig <- res[which(res$padj < 0.01), ]	# 13,409
